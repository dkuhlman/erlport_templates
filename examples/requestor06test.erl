...
run(NumProcesses, Tasks, PythonModule) ->
    Processes = make_processes(NumProcesses, [], PythonModule),
...
make_processes(0, Processes, _) -> Processes;
make_processes(N, Processes, PythonModule) ->
    SupervisorPid = self(),
    F1 = fun() -> erl_process(SupervisorPid, N, PythonModule) end,
    Pid = spawn(F1),
    make_processes(N - 1, [Pid|Processes], PythonModule).
...
erl_process(SupervisorPid, NProcess, PythonModule) ->
...
worker_process_loop(SupervisorPid, Port, NProcess) ->
    receive
        ...
        {Port, {data, Data}} ->
            ...
            case Data1 of
                {task_response, RequestCode, ReturnNProcess, ReturnNTask, ReturnData} ->
                    %
                    % We've received results/response from the Python process.
                    % Do your result processing here.
                    %
                    process_result(RequestCode, ReturnNProcess, ReturnNTask, ReturnData),
